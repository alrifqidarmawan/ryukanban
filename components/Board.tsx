...
export default function Board() {
  const { board, addList, moveCard, moveList } = useBoard();
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (isColumn) {
      if (activeId !== overId) {
        moveList(String(activeId), String(overId));
      }
      return;
    }
    ...
    if (startList.id === endList.id) {
      const activeIndex = startList.cards.findIndex(
      ...
      const newStartCards = startList.cards.filter(
        (card) => card.id !== activeId,
      );
